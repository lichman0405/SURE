...
- **Progress:** 70 of 166 v0.1 tasks accepted (phase P7 in progress).
...
- Finding model: four-level severity (`MustFix`, `ShouldFixFirst`, `CanFixLater`, `Note`), `AssessmentSource`, `SeverityRationale`, concrete evidence anchors including intent/claim/model, and a builder that refuses unsupported `MustFix` sources.
