...
pub enum MemberManifest {
    /// There is a `package.json` in the member.
    Present,
    /// There is nothing at that name.
    Absent,
...
/// A workspace pattern that named nothing SURE could resolve.
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// Why a workspace pattern did not resolve.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum UnresolvedReason {
    ...
    ///
    /// **Check [`super::Discovery::is_complete`] before reporting this as "there
    /// is no such directory".** A walk that ran out of depth or budget reports
    /// nothing at the path it did not reach, and this is what that looks like
    /// from here.
    NoMatch,
}
...
fn resolve_members(
    ...
    let case = options.scan.case;
    // The project root is seeded as already-seen, so a pattern that names it —
    // `"."`, or a `*` that reaches it — does not make the root a member of
    // itself. The root's manifest is `NodeProject::manifest`; describing it
    // twice would also read it twice against a finite budget.
    let mut seen: BTreeSet<String> = BTreeSet::new();
    seen.insert(read::lookup_key(Path::new(""), case));

    let mut ordered: Vec<PathBuf> = Vec::new();
    for pattern in &workspaces.patterns {
        match expand_pattern(tree, pattern, case) {
            Err(reason) => workspaces.unresolved.push(Unresolved {
                pattern: pattern.clone(),
                reason,
            }),
            Ok(paths) if paths.is_empty() => workspaces.unresolved.push(Unresolved {
                pattern: pattern.clone(),
...
/// The directories one workspace pattern names.
///
/// # Errors
///
