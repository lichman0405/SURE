...
/// has used before needs no removal. Uniqueness comes from `create_dir`, not from
/// the name, so two processes given the same id cannot collide — a directory that
/// exists is skipped rather than adopted.
struct Fixture {
    project: PathBuf,
}
