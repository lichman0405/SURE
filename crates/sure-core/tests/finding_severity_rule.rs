...
/// them is this crate's: clearing a fixed path and treating it as fresh fails on
/// Windows, and the test then describes a directory that was never emptied.
/// Nothing is adopted — a directory is taken with `create_dir`, which fails when
...
impl Scratch {
    fn new(test: &str) -> Self {
        Self {
            project: sure_testkit::scratch::directory("finding severity rule controls", test),
        }
    }
