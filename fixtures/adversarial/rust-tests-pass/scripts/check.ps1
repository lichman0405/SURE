...
# passes, so that code is zero.

$source = Split-Path -Parent $PSScriptRoot
$scratch = Join-Path ([System.IO.Path]::GetTempPath()) (
    'sure-rust-tests-pass-' + [guid]::NewGuid().ToString('N')
)
...
try {
    ...
    Write-Output ''
    Write-Output 'fixture                            : rust-tests-pass'
    ...
    Write-Output "verdict of this project's own check: $verdict (on purpose)"
}
finally {
