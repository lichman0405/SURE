...
# Read the JSON payload Claude Code sends to this hook process. There is no
# `param` block and `$input` is read before the console, which is the shape
...
# unchanged so Claude Code sees any decision/response SURE produces.
$inputJson | & $bin hook ingest --source claude-code @args
exit $LASTEXITCODE
