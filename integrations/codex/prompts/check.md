...
Run `sure check` for the current project using its configured execution/privacy mode. Summarize SURE's result faithfully. Do not turn `unknown`, `skipped`, `error`, or `cannot_confirm` into a pass, and do not drop them from the summary. Do not invent checks that did not run.
