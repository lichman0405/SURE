...
Run `sure repair` for the current project to obtain the repair contract. Treat that contract as the acceptance contract. Load it and implement only the required repair while preserving listed behavior. Run the acceptance checks. Then invoke `sure recheck`. Do not mark the SURE finding resolved from your own statement alone: your completion message is not evidence.
