# SURE Cursor plugin per-user install. No admin required.
param([switch]$ForceCopy)
$ErrorActionPreference='Stop'
# Resolve SURE binary (same order as hook launcher): SURE_BIN -> PATH -> LOCALAPPDATA
$bin=$env:SURE_BIN
...
# Cursor plugin dir: CURSOR_PLUGIN_DIR -> %APPDATA%\Cursor\plugins
...
$link=$false
if(-not $ForceCopy){
    $t=Join-Path $env:TEMP "sure-symlink-test-$([Guid]::NewGuid())"
