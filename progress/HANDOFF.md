...
28. A worker agent completed `P8-T005` — *Implement observed ProjectIntent capture
    path* — as commit `3bcc53d`. The supervisor verified all quality gates and
    accepted the task. Harness `user.request` events are captured as
    `ObservedUserRequest` requirements only when full recording is granted, the
    prompt text is redacted before storage, and the raw event is retained only
    under explicit `FullRecordingConsent::Full`.
29. A worker agent completed `P8-T006` — *Implement agent completion-claim
    extraction* — as commit `0c6a43a`. The supervisor verified all quality gates
    and accepted the task. Harness `agent.claim` events are validated, their
    `claim_text` is redacted, and a `Claim` document with `assessment:
    cannot_confirm` and original event provenance is stored.
30. A worker agent completed `P8-T007` — *Implement deterministic claim checkers*
    — as commit `66470ed`. The supervisor verified all quality gates and
    accepted the task. Stored `Claim` documents for `test_ran`, `file_changed`,
    `git_state`, and `current_code` are checked against recorded harness events
...
32. A worker agent completed `P8-T009` — *Implement capability/blind-spot reporting*
    ...
    payloads.
33. A worker agent completed `P8-T010` — *Implement AI-claim report section* — and
    the supervisor verified all quality gates and accepted the task. Checked agent
    claims flow into `ProjectVerdict::claim_checks` and are rendered in terminal,
    Markdown, HTML, and JSON reports with escaped attacker-controlled text,
    assessment labels (`Confirmed`/`Contradicted`/`CannotConfirm`/`NotCheckable`),
    and a plain-language reason or fallback explanation. The JSON report schema
    version advanced to 2 with an optional `claim_checks` array.
34. A worker agent completed `P8-T011` — *Implement idempotent/concurrent hook
    event ingestion* — as commit `5bf6ee1`. The supervisor verified all quality
    gates and accepted the task. A unique index on `(harness_session_id,
    project_root, harness_source)` makes concurrent session creation deterministic,
    `SessionEventStore::persist` inserts into `session_events` with `INSERT OR
    IGNORE` before creating the `records` row so duplicate event IDs return
    `AlreadyExists`, and new unit tests verify duplicate-event idempotency,
    concurrent session reuse, and distinct sessions for events without a harness
    session id.
35. A worker agent completed `P9-T001` — *Implement RepairContract domain/schema*
    — as commit `0982310`. The supervisor verified all quality gates and accepted
    the task. `RepairContract` already carried all required fields in
    `sure_domain::vocabulary`; this change completes `schemas/repair.schema.json`
    to declare `id` and `recheck`, and adds a conformance test ensuring the
    serialized contract carries both.
36. A worker agent completed `P9-T002` — *Generate repair contract from grounded
    finding* — as commit `a398107`. The supervisor verified all quality gates and
    accepted the task. `RepairContract::from_finding` in
    `crates/sure-domain/src/repair.rs` rejects ungrounded findings and empty
    re-check lists, copies the finding's evidence without inventing new facts, and
    produces observable acceptance conditions tied to the supplied re-checks.
37. A worker agent completed `P9-T003` — *Implement harness-neutral repair
    envelope* — as commit `d1fd41b`. The supervisor verified all quality gates and
    accepted the task. `RepairEnvelope` in `crates/sure-domain/src/repair.rs`
    wraps a `RepairContract` with a version and optional `target_harness`, so the
...
41. A worker agent completed `P10-T001` — *Validate current Claude Code
    plugin/hook schemas* — as commit `76f5464`. The supervisor verified all
    quality gates and accepted the task. The Claude Code plugin manifest was
    updated to reference commands, hooks and MCP files, and synthetic raw event
    fixtures for `SessionStart`, `PreToolUse`, `PostToolUse` and `Stop` were
    added under `integrations/claude-code/fixtures/`.
42. A worker agent completed `P11-T001` — *Validate current Cursor Plugin/hooks
...
- `crates/sure-core/src/harness_event.rs` (new) — versioned harness event ingestion.
- `IngestedEvent` holds the parsed `EventEnvelope`, agreed protocol version, and
  document kind.
- `IngestError` gives plain-language diagnostics for I/O failures, invalid JSON,
  missing version fields, version mismatch (with direction from `handshake::negotiate`),
  schema violations, and malformed envelopes.
- `ingest_event(reader)` and `ingest_event_str(text)` entry points.
- Builds on `EventEnvelope::from_json` and `handshake::negotiate` rather than
  duplicating version logic.
- 8 inline tests cover valid event round-trip, older/newer version refusal,
  malformed JSON, unknown fields, missing required fields, payload preservation,
...
- `crates/sure-core/src/store/sql/0003_sessions_unique.sql` (new) — unique index
  `sessions_unique` on `(harness_session_id, project_root, harness_source)`.
- `crates/sure-core/src/store/migrations.rs` — migration 0003 wired into the
  append-only migration list.
- `crates/sure-core/src/session_event_store.rs`:
  - `find_session_by_harness` helper extracted for re-querying sessions by
    harness id + project root + source.
  - `find_or_create_session_in_tx` uses `INSERT OR IGNORE` and re-queries on
    conflict, so concurrent hook processes creating the same session reuse one
    row instead of failing.
  - `persist` writes to `session_events` first with `INSERT OR IGNORE`, then
    creates the `records` row only when the event is new, and updates
    `record_row_id`. A duplicate `event_id` therefore returns
    `PersistResult::AlreadyExists` deterministically.
  - Collapsed nested `if let` checks with let-chains to satisfy clippy.
- Three new inline tests:
  - `concurrent_duplicate_event_id_is_idempotent` — two threads ingest the same
    event id; one returns `Stored`, the other `AlreadyExists`, and the database
    contains exactly one event row.
  - `concurrent_session_creation_reuses_one_session_row` — two threads ingest
    different events for the same harness session; one session row is created
    and both events are bound to it.
  - `events_without_session_id_get_distinct_sessions` — events with no harness
    session id each get their own session row because SURE has no correlation
    basis.

## Validation of `P8-T011`
...
- `crates/sure-domain/src/vocabulary.rs` — `RepairContract` already carried all
  required fields: `id`, `issue_id`, `problem`, `why_it_matters`, `required_fix`,
  `preserve`, `acceptance`, `recheck`, `forbidden_shortcuts`, and `evidence`.
- `schemas/repair.schema.json` — completed to declare the full wire surface:
  `id`, `issue_id`, `problem`, `why_it_matters`, `required_fix`, `preserve`,
  `acceptance`, `recheck`, `evidence`, and `forbidden_shortcuts`.
- `crates/sure-protocol/tests/conformance.rs` — new regression test
  `a_repair_contract_carries_its_identity_and_recheck_list` asserts that a
  serialized `RepairContract` exposes its own `id` and a non-empty `recheck`
  array, matching both the schema and the `is_actionable` rule.

## Validation of `P9-T001`
...
- `crates/sure-domain/src/repair.rs` (new) — `RepairContract::from_finding` and
  `RepairContract::from_finding_with_one_check`.
- `RepairFromFindingError` distinguishes two failure modes:
  - `NotGrounded` — the finding is not grounded in checkable evidence, so a
    contract would have to invent facts.
  - `NoRechecks` — no checks were supplied to re-run, so SURE could not observe
    whether the fix worked.
- The generated contract copies the finding's title, user impact, next step, and
  evidence; it does not add new evidence or invent new facts.
- Acceptance conditions are observable: re-run the supplied check(s) and confirm
  they pass; for observed-fact evidence with a concrete anchor, the fix must also
  be visible at that anchor.
- Forbidden shortcuts warn against test-only fixes for deterministic-check
  findings and symptom-hiding for observed-fact findings.
- `crates/sure-domain/src/lib.rs` — exposes `pub mod repair`.
- Inline tests cover grounded-finding generation, ungrounded rejection, empty
  re-check rejection, observable acceptance, no invented evidence, and
  claim-contradiction findings.

## Validation of `P9-T002`
...
- `crates/sure-domain/src/repair.rs` — `RepairEnvelope` struct and helpers.
  - `REPAIR_ENVELOPE_VERSION` is `1`.
  - `RepairEnvelope::new` wraps a `RepairContract` in a harness-neutral envelope.
  - `RepairEnvelope::for_harness` addresses the envelope to a specific adapter
    without changing the contract.
  - `RepairEnvelope::into_contract` returns the bounded contract every adapter
    works from.
  - `target_harness` is optional and omitted from the JSON wire form when `None`.
- `schemas/repair-envelope.schema.json` (new) — requires `version` and `contract`
  at the top level and permits `target_harness`.
- `crates/sure-protocol/src/documents.rs` — added `DocumentKind::RepairEnvelope`
  with schema file `repair-envelope.schema.json`, updated `ALL`, `as_str`,
  `file_name`, `schema_text`, and required-key parsing.
- `crates/sure-core/src/store/record.rs` — added
  `RecordKind::Document(DocumentKind::RepairEnvelope)` to `RecordKind::ALL` and
  updated the module comment from "six schemas" to "seven schemas" so the drift
  guard that insists every schema has a storable kind stays green.
- `crates/sure-protocol/tests/common/mod.rs` — added `repair_envelope()` fixture
  addressed to `claude-code` and included it in `documents()`.
- `crates/sure-protocol/tests/round_trip.rs` — added `round_trips(&repair_envelope())`.
- Inline tests cover wrapping without changing the contract, addressing to a
...
- `integrations/claude-code/.claude-plugin/plugin.json` — updated to reference
  `commands/`, `hooks/hooks.json` and `.mcp.json`, matching current Claude Code
  plugin conventions.
- `integrations/claude-code/fixtures/` (new directory) — four synthetic raw
  event fixtures showing the JSON payloads Claude Code hooks would forward to
  SURE: `session-start.json`, `pre-tool-use.json`, `post-tool-use.json`,
  `stop.json`. Each fixture includes `event`, `harness_session_id` and
  `source: "claude-code"`.
- `crates/sure-testkit/tests/integration_thinness.rs` — added
  `claude_code_synthetic_fixtures_are_valid_json_with_expected_shape`, which
  asserts every `.json` fixture is valid, is an object, contains the required
  keys, and declares `source` as `claude-code`.

## Validation of `P10-T001`
